...
# alias ls='eza --icons --level=0 --tree --sort=Name'
...
# alias htb-target='function _htb_target() { echo "$1" > ~/.config/waybar/current-htb-ip && echo "🎯 HTB IP set to $1"; }; _htb_target'

# CUSTOM
alias htb-target='function _htb_target() {
  echo "{\"ip\": \"$1\", \"started\": \"$(date --iso-8601=seconds)\"}" > ~/.config/waybar/htb-box.json
  echo "$1" > ~/.config/waybar/current-htb-ip
  echo "🎯 HTB IP set to $1"
}; _htb_target'
